...
var scrollToMessage = function (scrolled, isOwnSender) {
    ...
    if (scrolled) {
        ...
        var $scope_1 = angular.element($('body')).scope();
        $scope_1.$apply(function () {
            $scope_1.newMessages++;
        });
    }
};
...
$('#chatForm').submit(function () {
    ...
    if ($('#enterMessage').val() !== '') {
        var url_1;
        var text_1 = $('#enterMessage').val();
        if (text_1.startsWith('http://') === false && text_1.startsWith('https://') === false) {
            url_1 = 'http://' + text_1;
        }
        else {
            url_1 = text_1;
        }
        imageExists(url_1, function (exists) {
            if (exists) {
                scrollAtBottom = ($('#chat')[0].scrollHeight - $('#chat').scrollTop() === $('#chat').outerHeight()) ? true : false;
                $('#messages').append($('<li class="rightAlign">').html('<img style=\'margin: 10px; max-width: 400px;\' onerror=\'imgError(this);\' src=\'' + url_1 + '\' /> - <span style="color: rgb(255, 99, 0)">you</span>, ' + todisplay));
                var img = $('#messages li:last-child img');
                scrollToMessage(scrollAtBottom, true);
                setTimeout(function () {
                    if (img.width() === 0) {
                        socket.emit('chat message', { text: text_1, name: name, time: todisplay });
                        $('#messages li:last-child').html(text_1 + ' - ' + '<span style="color: rgb(255, 99, 0)">you</span>');
                    }
                    else {
                        socket.emit('image message', { url: url_1, name: name, time: todisplay });
                    }
                }, 50);
            }
            else if (isYouTube(url_1) !== 'no') {
                socket.emit('youtube message', { url: url_1, name: name, time: todisplay });
                scrollAtBottom = ($('#chat')[0].scrollHeight - $('#chat').scrollTop() === $('#chat').outerHeight()) ? true : false;
                $('#messages').append($('<li class="rightAlign">').html('<iframe style=\'margin: 10px;\' src=\'' + isYouTube(url_1) + '\' /> - ' + '<span style="color: rgb(255, 99, 0)">you</span>, ' + todisplay));
                scrollToMessage(scrollAtBottom, true);
            }
            else {
                socket.emit('chat message', { text: text_1, name: name, time: todisplay });
                scrollAtBottom = ($('#chat')[0].scrollHeight - $('#chat').scrollTop() === $('#chat').outerHeight()) ? true : false;
                $('#messages').append($('<li class="rightAlign">').html(text_1 + ' - ' + '<span style="color: rgb(255, 99, 0)">you</span>, ' + todisplay));
                scrollToMessage(scrollAtBottom, true);
            }
        });
        $('#enterMessage').val('');
...
$('#chat').scroll(function () {
    var scrolled = ($('#chat')[0].scrollHeight - $('#chat').scrollTop() === $('#chat').outerHeight()) ? true : false;
    if (scrolled) {
        var $scope_2 = angular.element($('body')).scope();
        $scope_2.$apply(function () {
            $scope_2.newMessages = 0;
        });
    }
});
